...
float timestep(const t_param params, t_speed *restrict cells, t_speed *restrict tmp_cells, int *restrict obstacles)
{
  ...
  const float c_sq = 3.f / 1.f; /* square of speed of sound */
...
#pragma omp simd
    for (int ii = 0; ii < params.nx; ii++)
    {
      ...
      ** respecting periodic boundary conditions (wrap around) */
      const int y_n = (jj != params.ny) ? (jj + 1) : 0;
      const int x_e = (ii != params.nx) ? (ii + 1) : 0;
      const int y_s = (jj != 0) ? (jj - 1) : (jj + params.ny - 1);
      const int x_w = (ii != 0) ? (ii - 1) : (ii + params.nx - 1);
      ...
      ** scratch space grid */
      ...
      if (!obstacles[ii + jj * params.nx])
      {
        ...
        local_density = cells->speeds0[ii + jj * params.nx]+ cells->speeds1[x_w + jj * params.nx] + cells->speeds2[ii + y_s * params.nx] + cells->speeds3[x_e + jj * params.nx] +  cells->speeds4[ii + y_n * params.nx] + cells->speeds5[x_w + y_s * params.nx] + cells->speeds6[x_e + y_s * params.nx] + cells->speeds7[x_e + y_n * params.nx] + cells->speeds8[x_w + y_n * params.nx];

        /* compute x velocity component */
        float u_x = (cells->speeds1[x_w + jj * params.nx] + cells->speeds5[x_w + y_s * params.nx] + cells->speeds8[x_w + y_n * params.nx] - (cells->speeds3[x_e + jj * params.nx] + cells->speeds6[x_e + y_s * params.nx] + cells->speeds7[x_e + y_n * params.nx])) / local_density;
        /* compute y velocity component */
        float u_y = (cells->speeds2[ii + y_s * params.nx] + cells->speeds5[x_w + y_s * params.nx] + cells->speeds6[x_e + y_s * params.nx] - (cells->speeds4[ii + y_n * params.nx] + cells->speeds7[x_e + y_n * params.nx] + cells->speeds8[x_w + y_n * params.nx])) / local_density;
        ...
        d_equ[0] = w0 * local_density * (1.f - u_sq * (0.5f * c_sq));
        /* axis speeds: weight w1 */
        d_equ[1] = w1 * local_density * (1.f + u[1] * c_sq + (u[1] * u[1]) * (0.5f * c_sq * c_sq) - u_sq * (0.5f * c_sq));
        d_equ[2] = w1 * local_density * (1.f + u[2] * c_sq + (u[2] * u[2]) * (0.5f * c_sq * c_sq) - u_sq * (0.5f * c_sq));
        d_equ[3] = w1 * local_density * (1.f + u[3] * c_sq + (u[3] * u[3]) * (0.5f * c_sq * c_sq) - u_sq * (0.5f * c_sq));
        d_equ[4] = w1 * local_density * (1.f + u[4] * c_sq + (u[4] * u[4]) * (0.5f * c_sq * c_sq) - u_sq * (0.5f * c_sq));
        /* diagonal speeds: weight w2 */
        d_equ[5] = w2 * local_density * (1.f + u[5] * c_sq + (u[5] * u[5]) * (0.5f * c_sq * c_sq) - u_sq * (0.5f * c_sq));
        d_equ[6] = w2 * local_density * (1.f + u[6] * c_sq + (u[6] * u[6]) * (0.5f * c_sq * c_sq) - u_sq * (0.5f * c_sq));
        d_equ[7] = w2 * local_density * (1.f + u[7] * c_sq + (u[7] * u[7]) * (0.5f * c_sq * c_sq) - u_sq * (0.5f * c_sq));
        d_equ[8] = w2 * local_density * (1.f + u[8] * c_sq + (u[8] * u[8]) * (0.5f * c_sq * c_sq) - u_sq * (0.5f * c_sq));
      ...
      /**************************************** */
      /**REBOUND: */
      /**If cell contains an obstacle */
      else
      {
        /* called after propagate, so taking values from scratch space
        ** mirroring, and writing into main grid */
        // float tmp1 = tmp_cells->speeds1[ii + jj * params.nx];
        // float tmp2 = tmp_cells->speeds2[ii + jj * params.nx];
        // float tmp6 = tmp_cells->speeds6[ii + jj * params.nx];
        // float tmp5 = tmp_cells->speeds5[ii + jj * params.nx];
...
float total_density(const t_param params, t_speed *cells)
{
  ...
  for (int jj = 0; jj < params.ny; jj++)
  {
    for (int ii = 0; ii < params.nx; ii++)
    {
      total = cells->speeds0[ii + jj * params.nx] + cells->speeds1[ii + jj * params.nx] + cells->speeds2[ii + jj * params.nx] + cells->speeds3[ii + jj * params.nx] + cells->speeds4[ii + jj * params.nx] + cells->speeds5[ii + jj * params.nx] + cells->speeds6[ii + jj * params.nx] + cells->speeds7[ii + jj * params.nx] + cells->speeds8[ii + jj * params.nx];
    }
  }
